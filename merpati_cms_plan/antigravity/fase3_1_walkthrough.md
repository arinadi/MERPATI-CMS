...
## Next: Fase 3 (Database & API)
### Fase 3.1 (Complete)
- Set up **Neon Postgres** connection configuration.
- Instantiated **Drizzle ORM** with `@neondatabase/serverless` HTTP driver.
- Wrote full relational schema in `db/schema.ts` supporting Auth.js, Posts (with revisions/meta), Terms (Categories/Tags), Media Library, and Settings.
- Added `db:generate`, `db:push`, and `db:studio` scripts to `package.json`.

*(Paused here. Waiting for user to supply `DATABASE_URL` in `.env` before running `db:push` to construct the tables in the database.)*
